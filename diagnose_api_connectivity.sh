    ...
    if [ "$WAZUH_STATUS" != "PORT_CLOSED" ] && [ "$WAZUH_STATUS" != "HTTPS_FAILED" ]; then
        ...
        if [ -z "${WAZUH_API_USER:-}" ] || [ -z "${WAZUH_API_PASSWORD:-}" ]; then
            ...
            AUTH_RESPONSE=$(curl $CURL_OPTS -u "${WAZUH_API_USER}:${WAZUH_API_PASSWORD}" \
                -X GET "${WAZUH_API_URL}/security/user/authenticate" 2>&1)
            
            if echo "$AUTH_RESPONSE" | grep -q "token"; then
                log_success "Wazuh authentication successful"
                WAZUH_TOKEN=$(echo "$AUTH_RESPONSE" | grep -o '"token":"[^"]*"' | cut -d'"' -f4)
                ...
                AGENTS_RESPONSE=$(curl $CURL_OPTS -H "Authorization: Bearer $WAZUH_TOKEN" \
                    -X GET "${WAZUH_API_URL}/agents?limit=5" 2>&1)
                
                if echo "$AGENTS_RESPONSE" | grep -q "affected_items"; then
                    AGENT_COUNT=$(echo "$AGENTS_RESPONSE" | grep -o '"total_affected_items":[0-9]*' | cut -d':' -f2)
...
EOF
            ...
            if echo "$AUTH_RESPONSE" | grep -q '"result"'; then
                ...
                ZABBIX_TOKEN=$(echo "$AUTH_RESPONSE" | grep -o '"result":"[^"]*"' | cut -d'"' -f4)
...
EOF
                ...
                if echo "$HOSTS_RESPONSE" | grep -q '"result"'; then
                    HOST_COUNT=$(echo "$HOSTS_RESPONSE" | grep -o '"hostid"' | wc -l)
                ...
                if echo "$AUTH_RESPONSE" | grep -qi "incorrect.*password"; then
...
if docker ps --format '{{.Names}}' | grep -q 'backend'; then
    log_success "Backend container đang chạy"
    
    # Check logs for Wazuh connection errors
    log_info "Kiểm tra logs cho lỗi Wazuh connection..."
    WAZUH_ERRORS=$(docker logs $(docker ps --filter "name=backend" --format "{{.Names}}" | head -1) 2>&1 | \
        grep -i "wazuh" | grep -iE "error|failed|refused|timeout" | tail -5)
    ...
    if [ "${ZABBIX_ENABLED:-false}" = "true" ]; then
        ...
        ZABBIX_ERRORS=$(docker logs $(docker ps --filter "name=backend" --format "{{.Names}}" | head -1) 2>&1 | \
            grep -i "zabbix" | grep -iE "error|failed|refused|timeout" | tail -5)
    ...
    COLLECTOR_LOGS=$(docker logs $(docker ps --filter "name=backend" --format "{{.Names}}" | head -1) 2>&1 | \
        grep "collector" | tail -10)
    
    if echo "$COLLECTOR_LOGS" | grep -q "collector_starting"; then
    ...
    if echo "$COLLECTOR_LOGS" | grep -q "collector_stats"; then
        ...
        LAST_STATS=$(echo "$COLLECTOR_LOGS" | grep "collector_stats" | tail -1)
...
if docker ps --format '{{.Names}}' | grep -q 'db'; then
    log_success "Database container đang chạy"
    ...
    if [ "$WAZUH_EVENTS" -gt 0 ]; then
        ...
        LATEST_EVENT=$(docker exec $DB_CONTAINER psql -U postgres -d ${POSTGRES_DB:-mini_soc_prod} \
            -c "SELECT event_timestamp, severity, agent_name FROM wazuh_events ORDER BY event_timestamp DESC LIMIT 1;" 2>/dev/null)
...
if [ "$WAZUH_STATUS" = "OK" ] && [ "$WAZUH_EVENTS" -gt 0 ]; then
    ...
    echo "     bash fix_data_flow_complete.sh"
